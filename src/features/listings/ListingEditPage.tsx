...
export function ListingEditPage({ id }: { id: string }) {
  ...
  return <main className="app-shell"><section className="profile-page"><a href={`#/listing/${id}`}>返回商品</a><h1>編輯商品</h1><form className="profile-form" onSubmit={submit}><label>價格<input value={editable.listingPrice} onChange={(e) => change({ listingPrice: Number(e.target.value) })} /></label><label>剩餘數量<input value={editable.remainingQuantity} onChange={(e) => change({ remainingQuantity: Number(e.target.value) })} /></label><label>替換商品圖片（1–3 張）<input type="file" accept="image/*" multiple onChange={(e) => setFiles(Array.from(e.target.files ?? []))} /></label><label><input type="checkbox" checked={editable.hasSleeve} onChange={(e) => change({ hasSleeve: e.target.checked })} />包手</label><label><input type="checkbox" checked={editable.supportsMyShip} onChange={(e) => change({ supportsMyShip: e.target.checked })} />賣貨便</label><label>備註<textarea value={editable.note ?? ''} onChange={(e) => change({ note: e.target.value || undefined })} /></label><button>儲存變更</button><button type="button" onClick={remove}>刪除商品</button>{error && <p role="alert">{error}</p>}{saved && <p role="status">已更新商品</p>}</form></section></main>;
}
