...
export function ListingPage({ id }: { id: string }) {
  ...
  return <main className="app-shell"><section className="profile-page"><a href="#">返回市集</a><h1>{name}</h1><p>{rarity}</p><div className="listing-images">{listing.imageUrls.map((url) => <img key={url} src={url} alt={`${name} 實卡照片`} />)}</div><p className="price">NT${listing.listingPrice.toLocaleString('zh-TW')} / 張</p><p>剩餘 {listing.remainingQuantity} 張</p><p>賣家：{seller?.displayName ?? '賣家'}</p>{seller && <p>聯絡方式：{seller.contactType} · {seller.contactValue}</p>}<p>{listing.hasSleeve && '包手 '}{listing.supportsMyShip && '賣貨便'}</p>{listing.note && <p>備註：{listing.note}</p>}{user?.uid === listing.sellerId && <a href={`#/listing/${id}/edit`}>編輯商品</a>}</section></main>;
}
