...
export function ListingPage({ id }: { id: string }) {
  ...
  return <main className="app-shell"><BackToMarketplaceLink /><article className="listing-page">
    ...
    <div className="listing-page-layout">
      ...
      <aside className="listing-purchase-panel"><p className="listing-price">NT${listing.listingPrice.toLocaleString('zh-TW')}<span>／張</span></p><p className="listing-stock">剩餘 {listing.remainingQuantity} 張</p><div className="listing-tags">{listing.hasSleeve && <span>包手</span>}{listing.supportsMyShip && <span>支援賣貨便</span>}</div><hr /><p className="seller-label">賣家</p><p className="seller-name">{seller?.displayName ?? '賣家'}</p>{seller ? <a className="contact-link" href={seller.contactType === 'line' ? `https://line.me/ti/p/~${seller.contactValue}` : undefined} target="_blank" rel="noreferrer">以 {seller.contactType} 聯絡：{seller.contactValue}</a> : <p>聯絡方式載入中</p>}{listing.note && <p className="listing-note">{listing.note}</p>}{user?.uid === listing.sellerId && <a className="edit-listing-link" href={`#/listing/${id}/edit`}>管理此商品</a>}</aside>
    </div>
  </article></main>;
}
