...
import { createListing, createListingId, getCard, getSellerProfile } from '../../data/firestore/repositories';
...
const initial: SellFormState = { cardId: '', files: [], listingPrice: '', quantity: '', hasSleeve: false, supportsMyShip: false, note: '' };
export function SellPage({ loadSellerProfile = getSellerProfile, loadCard = getCard }: { loadSellerProfile?: (uid: string) => Promise<SellerProfile | null>; loadCard?: (cardId: string) => Promise<Card | null> }) {
  ...
  useEffect(() => { if (!/^\d{4}$/.test(form.cardId)) { setCard(null); return; } let current = true; void loadCard(form.cardId).then((value) => { if (current) setCard(value); }).catch(() => { if (current) setCard(null); }); return () => { current = false; }; }, [form.cardId, loadCard]);
  async function submit(event: FormEvent) {
    ...
    if (!card || card.id !== result.values.cardId) { setErrors({ cardId: '找不到此卡片 ID，請確認後再試。' }); return; }
    setSaving(true); try { const id = createListingId(); const urls = await uploadListingImages(user.uid, id, result.values.files); const now = new Date(); const quantity = Number(result.values.quantity);
      await createListing({ id, sellerId: user.uid, cardId: card.id, characterName: card.characterName, rarity: card.rarity, imageUrls: urls, listingPrice: Number(result.values.listingPrice), originalQuantity: quantity, remainingQuantity: quantity, hasSleeve: result.values.hasSleeve, supportsMyShip: result.values.supportsMyShip, note: result.values.note || undefined, status: 'active', createdAt: now, updatedAt: now });
  ...
  return <main className="app-shell"><section className="profile-page"><a href="#/">返回市集</a><h1>刊登商品</h1><p>同版本、相近卡況才合併刊登。</p><form className="profile-form" onSubmit={submit} noValidate>
    <label>卡片 ID<input aria-label="卡片 ID" inputMode="numeric" maxLength={4} value={form.cardId} onChange={(e) => setForm({ ...form, cardId: e.target.value })} placeholder="例如 1096" /></label>{errors.cardId && <p role="alert">{errors.cardId}</p>}
    {card && <p role="status">角色／人名：{card.characterName}　稀有度：{card.rarity}</p>}
