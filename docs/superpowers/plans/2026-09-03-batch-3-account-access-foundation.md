...
firebase emulators:exec --project demo-conan-tcg-e2e --only auth,firestore,storage,functions "npx playwright test e2e/support/emulator-state.spec.ts --config playwright.config.ts --project=chromium"
firebase emulators:exec --project demo-conan-tcg-e2e --only auth,firestore,storage,functions "npx playwright test e2e/account-access.spec.ts --config playwright.config.ts --project=chromium"
```
